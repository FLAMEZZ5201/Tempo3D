        ...
        **kwargs,
        ...
        with synchronize_timer('Diffusion Sampling'):
            for i, t in enumerate(tqdm(timesteps, disable=not enable_pbar, desc="Diffusion Sampling:", leave=False)):
                ...
                # normalized time fraction for time-aware modules
                t_frac = float(i) / float(max(1, len(timesteps) - 1))
...
class Hunyuan3DDiTFlowMatchingPipeline(Hunyuan3DDiTPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # if extra_cond_tok is not None and append_extra_cond_tok:
        #     print("Adding extra cond tokens:", extra_cond_tok.shape) # [1, 1448, 1240]
        #     # zero_cond = torch.zeros(1, 1, cond["main"].shape[-1], dtype=cond["main"].dtype, device=cond["main"].device).half()
        #     extra_cond_tok = extra_cond_tok.float()
        #     # extra_cond_tok = torch.cat([extra_cond_tok[:,-1,:].view(1, 1, cond["main"].shape[-1]), zero_cond], dim=0).to(device)
        #     cond["main"] = torch.cat([extra_cond_tok, extra_cond_tok], dim=0) # [2, 1448, 1240]
        #
        #
        # print("Cond shape:", {k: v.shape for k, v in cond.items()})
